...
wk.register({
	["<leader>"] = {
		t = {
			...
			S = {
				"<cmd>lua require('telescope.builtin').lsp_dynamic_workspace_symbols({symbol_width = 50})<CR>",
				"Workspace lsp [S]ymbols",
			},
			d = {
				"<cmd>lua require'telescope.builtin'.find_files({cwd='~/nix_config'})<cr>",
				"Open NIX config [d]irectory",
			},
			v = {
...
wk.register({
	...
	["<leader>"] = {
		l = {
			...
			i = {
				string.format("<cmd>lua vim.lsp.buf.implementation(%s)<CR>", vertical_layout),
...
wk.register({
	["<leader>"] = {
		["-"] = { "<cmd>Yazi<cr>", "File manager open in current dir" },
		["_"] = { "<cmd>Yazi cwd<cr>", "Open the file manager in nvim's working directory" },
		["<c-up>"] = { "<cmd>Yazi toggle<cr>", "Resume last yazi session" },
